...
function letterFormat(num, precision = 3) {
   ...
   return `${m}${generateLetterSuffix(tier)}`;
}
...
function notationChooser(decimal, precision=3) {
   ...
   } else if (options.notation === 'alphabet'){
       return letterFormat(decimal, precision)
...
function notationChooserMinigame(decimal) {
   if (options.notation === 'infinity') {
       ...
       return letterFormat(decimal, precision=10)
   } else if (options.notation === 'blind'){
       return ""
   } else if (options.notation === 'birds array'){
